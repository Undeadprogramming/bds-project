...
public class PersonRepository {
        ...
        try (Connection connection = DataSourceConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "SELECT id_worker, first_name, middle_name, last_name, age, gender, position, salary" +
                             " FROM bds.worker w" +
                             " LEFT JOIN bds.worker_address wa ON w.id_address = wa.id_address" +
                             " LEFT JOIN bds.address a ON wa.id_address = a.id_address" +
                             " WHERE p.id_person = ?")) {
    ...
    public void createPerson(WorkerCreateView workerCreateView) {
        ...
        try (Connection connection = DataSourceConfig.getConnection();
            ...
            preparedStatement.setDouble(7, workerCreateView.getSalary());
